...
`);
    ...
    var O = /* @__PURE__ */ function() {
      ...
      return function(n, i, o, s, a, l) {
        var u, c, p, d, f, y, g, P, T, C, x, R, ce, J, Ke, $, z4, Ae, Y, tt, hr = n.constructor, An = n.s == i.s ? 1 : -1, Z = n.d, k = i.d;
        if (!Z || !Z[0] || !k || !k[0])
          return new hr(!n.s || !i.s || (Z ? k && Z[0] == k[0] : !k) ? NaN : Z && Z[0] == 0 || !k ? An * 0 : An / 0);
        for (l ? (f = 1, c = n.e - i.e) : (l = ge, f = b, c = ee(n.e / f) - ee(i.e / f)), Y = k.length, z4 = Z.length, T = new hr(An), C = T.d = [], p = 0; k[p] == (Z[p] || 0); p++)
          ;
        if (k[p] > (Z[p] || 0) && c--, o == null ? (J = o = hr.precision, s = hr.rounding) : a ? J = o + (n.e - i.e) + 1 : J = o, J < 0)
          C.push(1), y = true;
        else {
          if (J = J / f + 2 | 0, p = 0, Y == 1) {
            for (d = 0, k = k[0], J++; (p < z4 || d) && J--; p++)
              Ke = d * l + (Z[p] || 0), C[p] = Ke / k | 0, d = Ke % k | 0;
            y = d || p < z4;
          } else {
            for (d = l / (k[0] + 1) | 0, d > 1 && (k = e(k, d, l), Z = e(Z, d, l), Y = k.length, z4 = Z.length), $ = Y, x = Z.slice(0, Y), R = x.length; R < Y; )
              x[R++] = 0;
    ...
    var v = { manual: rn.Prism && rn.Prism.manual, disableWorkerMessageHandler: rn.Prism && rn.Prism.disableWorkerMessageHandler, util: { encode: function(e) {
      ...
      for (let g in r) {
        ...
        for (let T = 0; T < P.length; ++T) {
          ...
          C = C.pattern || C;
          for (let $ = n, z4 = i; $ < t.length; z4 += t[$].length, ++$) {
            let Ae = t[$];
            ...
            if (ce && $ != t.length - 1) {
              ...
              if (!p)
                break;
              var c = p.index + (R ? p[1].length : 0), d = p.index + p[0].length, a = $, l = z4;
              for (let k = t.length; a < k && (l < d || !t[a].type && !t[a - 1].greedy); ++a)
                l += t[a].length, c >= l && (++$, z4 = l);
              if (t[$] instanceof ye)
                continue;
              u = a - $, Ae = e.slice(z4, l), p.index -= z4;
            } else {
              C.lastIndex = 0;
              var p = C.exec(Ae), u = 1;
            }
            ...
            let Y = [$, u];
            f && (++$, z4 += f.length, Y.push(f));
            let tt = new ye(g, x ? v.tokenize(p, x) : p, Ke, p, ce);
            if (Y.push(tt), y && Y.push(y), Array.prototype.splice.apply(t, Y), u != 1 && v.matchGrammar(e, t, r, $, z4, true, g), o)
              break;
          }
        }
      }
...
var productsRoutes = async (app2) => {
  app2.get("/products", async () => {
  ...
  app2.get("/products/search", async (request) => {
    const querySchema = import_zod.z.object({
      q: import_zod.z.string().optional()
    });
    const { q: q2 } = querySchema.parse(request.query);
    const products = await prisma.product.findMany({
      where: {
        OR: [
          {
  ...
  app2.get("/products/:id", async (request) => {
    const paramsSchema = import_zod.z.object({
      id: import_zod.z.string()
    });
    const { id: id2 } = paramsSchema.parse(request.params);
  ...
  app2.post("/products", async (request) => {
    ...
    const { title, description, price, image, category, stock } = bodySchema.parse(request.body);
  ...
  app2.put("/products/:id", async (request) => {
    const paramsSchema = import_zod.z.object({
      id: import_zod.z.string()
    });
    const { id: id2 } = paramsSchema.parse(request.params);
  ...
  app2.delete("/products/:id", async (request) => {
    const paramsSchema = import_zod.z.object({
      id: import_zod.z.string()
    });
    const { id: id2 } = paramsSchema.parse(request.params);
...
var soldRoutes = async (app2) => {
  app2.get("/sales", async () => {
  ...
  app2.post("/sales", async (request) => {
    ...
    for (const productData of products) {
      const { title, price, category, id: id2 } = productData;
...
var ordersRoutes = async (app2) => {
  app2.get("/orders", async () => {
  ...
  app2.post("/order/:id", async (request) => {
